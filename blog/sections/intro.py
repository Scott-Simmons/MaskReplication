def tldr() -> str:
    return (
        "I replicate the MASK benchmark's headline result: scaling improves accuracy "
        "but not honesty. I also show various ways that the single honesty score hides "
        "nuance. Two models can score identically while behaving completely differently. "
        "I show why reporting the full basis is useful, and argue for reporting raw counts, error bars, and "
        "the full response basis, not just one number, as the way forward for deception "
        "evaluation."
    )


def intro() -> str:
    return "TODO: Insert intro"


def link_to_ai() -> str:
    return "TODO: Insert link_to_ai"


def how_i_reacted() -> str:
    return "TODO: Insert how_i_reacted"


def what_i_did() -> str:
    return "TODO: Insert what_i_did"
